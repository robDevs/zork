...
static int coordY_adv = 20;
static int coordX_adv = 10;
...
void psvDebugScreenVita2d(const char *_text) {
	...
	for(int i = 0; i < 3; i++) {
		vita2d_start_drawing();
		vita2d_pgf_draw_text(pgf, coordX_adv, coordY_adv, RGBA8(255,255,255,255), 1.0f, _text);
		vita2d_end_drawing();
		vita2d_swap_buffers();
	}

	if(*_text == '\n') {
		coordY_adv += vita2d_pgf_text_height(pgf, 1.0f, _text) /2;
		coordX_adv = 10;
	}
	else if(*_text == '\t') {
		coordX_adv += 20;
	}
	else coordX_adv += vita2d_pgf_text_width(pgf, 1.0f, _text);
}
